          ...
          return (
            <div key={state} className="space-y-2">
              {isRealState ? (
                ...
                <h4 className="font-bold text-red-600 text-sm border-b border-red-100 pb-1 uppercase tracking-wider">{state}</h4>
              )}
              <ul className="space-y-1">
                {cities.map(city => (
                  <li key={city}>
                    <Link prefetch={false} href={`/call-girls/${getCallGirlsSlug(city)}`}
