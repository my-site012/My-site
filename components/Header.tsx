  ...
  return (
    <header className="bg-white border-b-2 border-red-600 shadow-md sticky top-0 z-50">
      <div className="max-w-7xl mx-auto px-4 py-2 flex items-center justify-between gap-1 md:gap-3">
        {/* Logo */}
        <Link prefetch={false} href="/" className="text-xl md:text-3xl font-black tracking-tighter flex items-center gap-1 group shrink-0">
          ...
          {!loading && (
            <div className="flex gap-1 text-[10px] md:text-sm font-bold items-center">
              {user ? (
                ...
                <>
                  <Link prefetch={false} href="/login" className="text-gray-800 bg-white border border-gray-300 px-1.5 py-1 rounded-md hover:bg-gray-50 transition-all shadow-sm active:scale-95 whitespace-nowrap">Login</Link>
                  <Link prefetch={false} href="/signup" className="text-white bg-gradient-to-r from-red-600 to-red-700 px-2 py-1 rounded-md hover:from-red-700 hover:to-red-800 transition-all shadow-md border border-red-700 active:scale-95 whitespace-nowrap">Sign Up</Link>
                </>
              )}
            </div>
          )}
